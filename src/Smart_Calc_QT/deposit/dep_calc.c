...
void calc_percents_cap(const int term, const double rate, const double amount,
                       const double tax, const int k, const double *rep_data,
                       const double *with_data, double *my_return);
void calc_percents(const int term, const double rate, const double amount,
                   const double tax, const double *rep_data,
                   const double *with_data, double *my_return);
...
void dep_calc(const double amount, const double rate, const double tax,
  ...
  if (capitalization == 0) {
    ...
    if (type == 0)
      calc_percents_cap(term, rate, amount, tax, 1, rep_data, with_data,
                        my_return);
    if (type == 1)
      calc_percents_cap(term, rate, amount, tax, 3, rep_data, with_data,
                        my_return);
    if (type == 2)
      calc_percents_cap(term, rate, amount, tax, 12, rep_data, with_data,
                        my_return);
  }
  amount_of_taxes = 0.0;
}
...
void calc_percents_cap(const int term, const double rate, const double amount,
  ...
  for (int mounth_count = 1, number_mounth = 0; static_mounth_count != term;
       mounth_count++, number_mounth++, static_mounth_count++) {
