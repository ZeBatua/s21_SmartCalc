...
double read_string(char *start_string, int *break_status) {
  ...
  while (string_position < size) {
    if (get_num(start_string, &string_position, &current_num)) {
      ...
      if (*break_status == 4 &&
          check_power_next_function(start_string, string_position))
        continue;
    ...
    } else if (get_function(start_string, &string_position,
                            &current_function)) {
      previous_function = peek_function(f_head);
      push_function(&f_head, current_function);
      if (current_function == '(') {
        current_num =
            exec_expression_with_open_bracket(start_string, &string_position);
        pop_function(&f_head);
      ...
      if (string_position != 1) {
        if (is_lower_priority(previous_function, current_function,
                              &start_string[string_position - 1])) {
      ...
      if (peek_function(f_head) == '-')
        current_num =
            get_minus(start_string, &string_position, &n_head, &f_head);
      if (peek_function(f_head) == '/')
        current_num = get_div(start_string, &string_position, &n_head, &f_head);
    }
    if ((int)strlen(start_string) == string_position ||
        current_function == ')') {
      calc_current_values(&n_head, &f_head);
...
int check_power_next_function(char *part_string, int string_position) {
  int status = 0;
  int skip_status = 0;
  for (; status == 0 && &part_string[string_position] != NULL;
       string_position++) {
    if (part_string[string_position] == '^') {
      ...
      skip_status = 1;
    } else if (!is_num(part_string[string_position]) && skip_status == 1 &&
...
int is_needed_to_calc_now(char function) {
  int status = 0;
  if (function == 'c' || function == 'C' || function == 's' ||
      function == 'S' || function == 't' || function == 'T' ||
      function == 'l' || function == 'L' || function == 'q') {
    status = 1;
  }
  return status;
}
...
double exec_expression_with_division(char *curent_string,
                                     int *string_position) {
  double result = 0.0;
...
int is_lower_priority(char previous_function, char current_function,
  ...
  int status = 0;
  if (string != NULL) {
    if (((previous_function == '*' || previous_function == '/' ||
          previous_function == '^') &&
         (current_function == '-' || current_function == '+')) ||
        (is_math_operator(previous_function) && current_function != '(' &&
         current_function != '-' && !is_math_operator(current_function))) {
      status = 1;
    }
    ...
    if ((!is_needed_to_calc_now(current_function) &&
         is_needed_to_calc_now(previous_function)) &&
        !is_needed_to_calc_now(*(string - 1))) {
      status = 1;
    }
  }
  return status;
