...
int valid_string(char *input_string, char *input_x_string) {
  ...
  if (status == 0)
    status = check_valid_brackets(
        input_string);  // счетчик открывающих скобки операторов
  if (status == 0)
    status = valid_binary_op_position(
        input_string);  // проверка правильной позиции операторов
  if (status == 0)
    status = valid_node_position(
        input_string);  // проверка правильно посталвенной точки
  if (status == 0) status = enough_arguements(input_string);  // cos(?)
  // for common string
  if (status == 0)
    status = adapt_string(input_x_string);  // cos -> c  // +mini valid string
  if (status == 0) upgrade_string(input_x_string);  // 2cos -> 2*cos
  if (status == 0)
    status = check_valid_brackets(
        input_x_string);  // счетчик открывающих скобки операторов
  if (status == 0)
    status = valid_binary_op_position(
        input_x_string);  // проверка правильной позиции операторов
  if (status == 0)
    status = valid_node_position(
        input_x_string);  // проверка правильно посталвенной точки
  if (status == 0) status = enough_arguements(input_x_string);  // cos(?)
  if (status == 0)
    status = x_in_input_x_string(
        input_x_string);  // проверка что x не содержит x в самом себе
...
int valid_binary_op_position(char *string) {
  int status = 0;
  size_t length_string = strlen(string);
  if (is_binary_op(*string) && *string != '-')
    status = 1;  // проверка что первый знак не бинарный оператор (!-)
  if (length_string > 0) {
    if (is_binary_op(string[length_string - 1]))
      status = 1;  // проверка что последний знак не бинарный оператор
  }
  if (triple_binary_op(string))
    status = 1;  // провекра наличия 3 операторов подряд
  for (size_t i = 0; length_string > i && status != 1; i++) {
    if (&string[i + 1] != NULL) {
      if (is_open_op(string[i]) && is_binary_op(string[i + 1]) &&
          string[i + 1] != '-')
        status = 1;  // cos(*...) sin(/...)
    }
    if (&string[i - 1] != NULL) {
      if (string[i] == ')' && is_binary_op(string[i - 1]))
        status = 1;  // .../) ...*/)
    }
    if (i != length_string - 1) {
      if (is_invalid_double_op(&string[i]))
        status = 1;  // проверка коректных 2 операторов подоряд
      if (num_after_math_op(&string[i])) status = 1;
    }
    if (binary_op_after_math_op(&string[i]))
      status = 1;  // проверка случаев вида cos(* sin(/
    if (i != 0) {
...
int is_open_op(char string) {
  int status = 0;
  if (string == '(' || string == '^' || string == 's' || string == 'S' ||
      string == 'c' || string == 'C' || string == 't' || string == 'T' ||
      string == 'q' || string == 'l' || string == 'L') {
    status = 1;
  }
  return status;
}
...
int binary_op_after_math_op(char *string) {
  int status = 0;
  if ((string + 1) != NULL) {
    if (is_open_op(*string) && is_binary_op(*(string + 1)) &&
        *(string + 1) != '-')
      status = 1;
  }
  return status;
...
void upgrade_string(char *input_string) {
  ...
  for (size_t i = 0, j = 0; i < string_length; i++) {
    if (&input_string[i + 1] != NULL) {
      if ((is_num(input_string[i]) || input_string[i] == '.' ||
           input_string[i] == 'x') &&
          is_stuck_symbol(input_string[i + 1])) {  // 2cos -> 2*cos
  ...
  char nothing = '\0';
  memset(input_string, nothing, string_length);
  strcpy(input_string, new_string);
}

int is_stuck_symbol(char symbol) {
  int status = 0;
  if (symbol == 'c' || symbol == 'C' || symbol == 's' || symbol == 'S' ||
      symbol == '(' || symbol == 'l' || symbol == 'L' || symbol == 't' ||
      symbol == 'T' || symbol == 'q' || symbol == 'x') {
    status = 1;
  }
  return status;
}
...
int adapt_string(char *string) {
  ...
  for (size_t i = 0, j = 0; i < string_length; i++, j++) {
    if (ez_skip(string[i])) {
      ...
      i += 2;
               ...
               string[i + 2] == 'o' && string[i + 3] == 's') {  // acos
      new_string[j] = 'C';
      i += 3;
      ...
      i += 2;
               ...
               string[i + 2] == 'i' && string[i + 3] == 'n') {  // asin
      new_string[j] = 'S';
      i += 3;
      ...
      i += 2;
               ...
               string[i + 2] == 'a' && string[i + 3] == 'n') {  // atan
      new_string[j] = 'T';
      i += 3;
      ...
      i += 2;
               ...
               string[i + 2] == 'r' && string[i + 3] == 't') {  // sqrt
      new_string[j] = 'q';
      i += 3;
    } else if (string[i] == ' ') {
      j--;
  ...
  char nothing = '\0';
  memset(string, nothing, string_length);
